    ...
    <div className="modal-centered">
      ...
      {props.isAuthenticated === true ?
        ...
        <div>
          ...
          <p style={{ marginLeft: '7px' }} className={`bd-callout borderLeftColor${sectionDetails.key} `}>{formData.description.value}</p>
          <button type="button" className="btn btn-sm btn-secondary" onClick={props.cancelHandler} style={{ marginLeft: '7px', float: 'right' }}>Close</button>
        </div>
      }
    </div>
  )
